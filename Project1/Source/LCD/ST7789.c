...
static void LCD_Init_Backlight(void) {
	...
	PWM_Init(LCD_BL_TPM, LCD_BL_TPM_CHANNEL, LCD_BL_PERIOD, (7*(LCD_BL_PERIOD>>3)));	
...
//#if LESS_INLINING
//	static void LCD_24S_Write_Command(uint8_t command)__attribute__((always_inline));
//#endif
...
//#if LESS_INLINING
//	static void LCD_24S_Write_Data(uint8_t data)__attribute__((always_inline));
//#endif
...
/*static void my_LCD_24S_Write_Data(uint8_t data)
{
...
void LCD_Write_Rectangle_Pixel_mycode( uint32_t * pSrcR,uint32_t * pSrcG,uint32_t * pSrcB,unsigned int count )
{
	GPIO_SetBit(LCD_D_NC_POS);
	while (count--){
	uint32_t R,G,B,A,C;
uint8_t b1, b2;
	...
	//b1[1] = (R&0x0000f800) | ((G&0x0000e000)>>5);
	 //b2[1] = ((G&0x00001c00)<<3) | ((B&0x0000f800)>>3);
		//b1[2] = (R&0x00f80000) | ((G&0x00e00000)>>5);
	 //b2[2] = ((G&0x001c0000)<<3) | ((B&0x00f80000)>>3);
		//b1[3] = (R&0xf8000000) | ((G&0xe0000000)>>5);
	 //b2[3] = ((G&0x1c000000)<<3) | ((B&0xf8000000)>>3);
		A=(R&0xf8f8f8f8) | ((G&0xe0e0e0e0)>>5);
		C=((G&0x1c1c1c1c)<<3) | ((B&0xf8f8f8f8)>>3);
...
LCD_24S_Write_Data(A);
LCD_24S_Write_Data(C);
LCD_24S_Write_Data(A>>8);
LCD_24S_Write_Data(C>>8);
LCD_24S_Write_Data(A>>16);
LCD_24S_Write_Data(C>>16);
LCD_24S_Write_Data(A>>24);
LCD_24S_Write_Data(C>>24);
